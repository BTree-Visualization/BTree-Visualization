...
public class BPlusLeafNode<T>(int degree, BufferBlock<(Status status,
      ...
      : BTreeNode<T>(degree, bufferBlock)     
  {
    ...
    public BPlusLeafNode(int degree, int[] keys, T[] contents, BPlusLeafNode<T>? _NextNode, BPlusLeafNode<T>? _PrevNode,
    ...
    public override ((int, T), BTreeNode<T>) Split()
    {
      ...
      BPlusLeafNode<T> newNode = new(_Degree, newKeys, newContent, null, this, _BufferBlock);
